...
import { type CardCost, type PlayersAtCombinations, type Pot } from '../types';
...
export class Players {
    ...
    getNextActivePlayer(): Player {
        ...
        const nextPlayer = consecutivePlayer ?? playersLeftToReact[0];
        return nextPlayer;
    }
    ...
    getWinnersOfRound(cardsOnTheDesk: Card[]): Player[] {
    ...
    private calculateSidePots(): Pot[] {
        const players = [...this.playerList];
        const pots: Pot[] = [];
        let previousAllIn = 0;
        players.sort((a, b) => a.sumOfPersonalBetsInThisRound - b.sumOfPersonalBetsInThisRound).forEach(p => {
            if (p.isAllIn) {
                const pot: Pot = { name: `${p.name}-allIn`, money: 0, playersEligible: [] };
                const allInMonyByPlayer = p.sumOfPersonalBetsInThisRound;
        ...
        const extraPot: Pot = { name: 'extra-pot', money: 0, playersEligible: [] };

        players.forEach(player => {
            if (player.sumOfPersonalBetsInThisRound > previousAllIn) {
                const extraMoney = player.sumOfPersonalBetsInThisRound - previousAllIn;
    ...
    private getWinnerOfPot(potMoney: number, cardsOnTheDesk: Card[], playersEligibleInThisPot: Player[]): void {
        ...
        for (const combinationName of combinations) {
            ...
            const amountOfPlayersWithThisCombination = playersWithThisCombination?.length ?? 0;

            if (playersWithThisCombination === undefined || amountOfPlayersWithThisCombination === 0) {
                continue;
    ...
    private checkPlayersHandCombinations(players: Player[], cardsOnDesk: Card[]): PlayersAtCombinations {
        ...
        const playersWithHigheCard: Player[] = [];
        const playersWithOnePair: Player[] = [];
        const playersWithTwoPairs: Player[] = [];
        const playersWithThreeOfKind: Player[] = [];
        const playersWithFourOfKind: Player[] = [];
        const playersWithFullHouse: Player[] = [];
        const playersWithRoyalFlush: Player[] = [];
        const playersWithStraightFlush: Player[] = [];
        const playersWithFlush: Player[] = [];
        const playersWithStraight: Player[] = [];
        players.forEach(player => {
            ...
            const cardsInStraightFlush = (cardsWithFlush.length > 0) ? getCardsInStraightIfThereIsAny(cardsWithFlush) : [];

            // STRAIGHT_FLUSH AND ROYAL FLUSH
            if (cardsInStraightFlush.length > 0) {
    ...
    private resolveTieInSameCombination(playersWithSameCardCombination: Player[], potMoney: number): void {
    ...
    private splitPotBetweenPlayers(players: Player[], moneyInPot: number): void {
    ...
    private UpdateWinners(winners: Player[], player: Player, winMoney: number): void {
        const alreadyIsWinnerPlayerIndex = winners.findIndex(p => p.id === player.id);
        if (alreadyIsWinnerPlayerIndex > -1) {
            const winner = winners[alreadyIsWinnerPlayerIndex];
