...
import { type Pot, type CardCost, type PlayersAtCombinations } from '../types';
...
export class Players {
  ...
  private calculateSidePots (): Pot[] {
    ...
    players.sort((a, b) => a.sumOfPersonalBetsInThisRound - b.sumOfPersonalBetsInThisRound).forEach(p => {
      if (p.isAllIn) {
        const pot = { name: `${p.name}-allIn`, money: 0, playersEligible: [] };
    ...
    const extraPot = { name: 'extra-pot', money: 0, playersEligible: [] };
